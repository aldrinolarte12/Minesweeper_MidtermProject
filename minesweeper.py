...
def sounds(select):
    ...
    if select == 'bomb':
        PlaySound(resource_path(r'sounds\find_bomb.wav'), SND_FILENAME | SND_ASYNC)
    elif select == 'miss':
        PlaySound(resource_path(r'sounds\miss.wav'), SND_FILENAME | SND_ASYNC)
    elif select == 'end':
        PlaySound(resource_path(r'sounds\end.wav'), SND_FILENAME | SND_ASYNC)
    elif select == 'click':
        PlaySound(resource_path(r'sounds\click.wav'), SND_FILENAME | SND_ASYNC)
    elif select == 'click2':
        PlaySound(resource_path(r'sounds\click2.wav'), SND_FILENAME | SND_ASYNC)
    elif select == 'start':
        PlaySound(resource_path(r'sounds\start.wav'), SND_FILENAME | SND_ASYNC)
    elif select == 'exit':
        PlaySound(resource_path(r'sounds\exit.wav'), SND_FILENAME | SND_ASYNC)


def menu():
    global bombs_num, height, width, pl1_color, pl2_color, small_font, big_font, data

    with open(resource_path(r'data.json')) as file:
        data = json.load(file)
...
def setting():
    ...
    sett.iconbitmap(resource_path(r".\img\setting.ico"))
    sett.resizable(0, 0)
    ...
    for item in options['mode']:
        Radiobutton(mode_frame, text=item.title(), fg='white', bg='#172a3a', selectcolor='black',
                    activebackground='#172a3a', font=big_font, variable=mode_var, value=item,
                    command=lambda: sounds('click')).pack(ipadx=15, anchor=W)
    ...
    for item in options['color']:
        Radiobutton(pl1_frame, text=item.title(), fg=item, bg='#172a3a', selectcolor='black',
                    activebackground='#172a3a', font=big_font, variable=pl1_color_var, value=item,
                    command=change1).grid(row=0, column=col, ipadx=15)
        col += 1
    ...
    col = 0
    for item in options['color']:
        Radiobutton(pl2_frame, text=item.title(), fg=item, bg='#172a3a', selectcolor='black',
                    activebackground='#172a3a', font=big_font, variable=pl2_color_var, value=item,
                    command=change2).grid(row=0, column=col, ipadx=15)
        col += 1
    ...
    for item in options['font']:
        Radiobutton(font_frame, text=item, fg='white', bg='#172a3a', selectcolor='black', activebackground='#172a3a',
                    font=(item, '18'), variable=font_var, value=item, command=lambda: sounds('click')).grid(row=0,
                                                                                                            column=col,
                                                                                                            ipadx=5)
        col += 1
    ...
    on_off = Button(sounds_frame, width=6, text=data['sound'][0].upper(), bg=data['sound'][1], fg='white',
                    font=small_font, command=lambda: toggle(data['sound'][0]))
    on_off.pack(pady=5)
    ...
    Button(sett, text='MENU', bg='grey', fg='white', font=big_font, command=save).grid(row=2, column=3, padx=20,
                                                                                       pady=15, sticky=E + W + N + S)


def setUp():
    global bombs, field, goal
...
def start():
    ...
    main.resizable(0, 0)
    main.iconbitmap(resource_path(r"img\bomb.ico"))

    photo_bomb = PhotoImage(file=resource_path(r"img\bomb.png")).subsample(2, 2)
    photo_reset = PhotoImage(file=resource_path(r"img\reset.png")).subsample(2, 2)

    for i in range(height):
        for j in range(width):
            Button(main, bg='#172a3a', command=lambda i=i, j=j: play(i, j), padx=20, font=small_font).grid(row=i,
                                                                                                           column=j)
    ...
    switch_label = Label(main, text='Player 1\'s Turn', bg=pl1_color, font=big_font)
    switch_label.grid(row=height + 1, column=0, columnspan=width, sticky=E + W)
    goal_label = Label(main, text='Goal: Find {} Bombs to Win!'.format(goal), bg='#172a3a', fg='white',
                       font=big_font)
    goal_label.grid(row=height + 2, column=0, columnspan=width, sticky=E + W)
    menu_button = Button(main, text='M    E    N    U', bg='#172a3a', fg='black', font=("bold", 15), command=back)
    menu_button.grid(row=height + 3, column=0, columnspan=width, sticky=E + W + N + S)
    ...
    def end():
        ...
        Button(main, image=photo_reset, bg='white', fg='black', command=reset).grid(row=height + 3, column=width - 1,
                                                                                    sticky=E + W + N + S)

        for item in field.keys():
            if item not in used:
                if field[item] == -1:
                    Button(main, bg='white', image=photo_bomb, state=DISABLED, font=small_font).grid(row=item[0],
                                                                                                     column=item[1],
                                                                                                     sticky=E + W + S + N)
                elif field[item]:
                    Button(main, text='x', bg='red', fg="black", state=DISABLED, font=small_font).grid(row=item[0],
                                                                                                       column=item[1],
                                                                                                       sticky=E + W)
                else:
                    Button(main, bg='white', state=DISABLED, font=small_font).grid(row=item[0], column=item[1],
                                                                                   sticky=E + W)

        if pl1 >= goal or pl2 >= goal:
            if pl1 > pl2:
                winner = "Player 1"
                winner_color = pl1_color
            elif pl2 > pl1:
                winner = "Player 2"
                winner_color = pl2_color
            else:
                winner = "It's a tie!"
                winner_color = 'white'

            win_message = Label(main, text="Congratulations {}!".format(winner), bg=winner_color, font=big_font)
            win_message.grid(row=height + 1, column=0, columnspan=width, sticky=E + W)

    def findzero(i, j):
        if field[(i, j)]:
            used.append((i, j))
            checked.append((i, j))
            Button(main, text='x', bg='red', fg="black", state=DISABLED, font=small_font).grid(row=i, column=j,
                                                                                               sticky=E + W)
            return
        elif (i, j) in checked:
            return
    ...
    def play(row, col):
        ...
        if field[(row, col)] == -1:
            ...
            Button(main, bg=pl1_color if flag else pl2_color, image=photo_bomb, state=DISABLED, padx=20,
                   font=small_font).grid(row=row, column=col, sticky=E + W + S + N)
            ...
            if field[(row, col)] in {1, 2, 3, 4, 5, 6, 7, 8, 9}:  # Including numbers beyond 1 to 5
                Button(main, text='x', bg='red', fg="black", state=DISABLED, font=small_font).grid(row=row,
                                                                                                   column=col,
                                                                                                   sticky=E + W)
            else:
                Button(main, text=field[(row, col)], bg='red', fg="black", state=DISABLED, font=small_font).grid(
                    row=row,
                    column=col,
                    sticky=E + W)
        else:
            ...
            Button(main, text='x', bg='red', fg="black", state=DISABLED, font=small_font).grid(row=row, column=col,
                                                                                               sticky=E + W)

        if flag:
            pl1_label['text'] = pl1
            switch_label['text'] = 'Player 1\'s Turn'  # Update turn label
            switch_label['bg'] = pl1_color
        else:
            pl2_label['text'] = pl2
            switch_label['text'] = 'Player 2\'s Turn'  # Update turn label
